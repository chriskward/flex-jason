...
Can be run standalone:
    python -m agent.codegen config.yaml pipeline.yaml
"""
...
def run(config: dict, pipeline: dict) -> dict:
    ...
    -------
    dict
        Updated pipeline with a ``codegen`` key added/modified.
    """
    pipeline.setdefault("codegen", {})
    return pipeline


def main():
    parser = argparse.ArgumentParser(description="Run the Code Generation pipeline stage")
