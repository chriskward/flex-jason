...
Can be run standalone:
    python -m agent.variables config.yaml pipeline.yaml
"""
...
def run(config: dict, pipeline: dict) -> dict:
    ...
    -------
    dict
        Updated pipeline with a ``variables`` key added/modified.
    """
    pipeline.setdefault("variables", {})
    return pipeline


def main():
    parser = argparse.ArgumentParser(description="Run the Variables pipeline stage")
