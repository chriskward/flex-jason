...
Can be run standalone:
    python -m agent.derivations config.yaml pipeline.yaml
"""
...
def run(config: dict, pipeline: dict) -> dict:
    ...
    -------
    dict
        Updated pipeline with a ``derivations`` key added/modified.
    """
    pipeline.setdefault("derivations", {})
    return pipeline


def main():
    parser = argparse.ArgumentParser(description="Run the Derivations pipeline stage")
